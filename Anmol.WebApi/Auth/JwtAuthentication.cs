...
namespace _Anmol.WebApi.Auth
{
    public class JwtAuthentication : Attribute, IAuthenticationFilter
    {
        ...
        public async Task AuthenticateAsync(HttpAuthenticationContext context, CancellationToken cancellationToken)
        {
            ...
            {               
                var JWToken = SessionHelper.AuthToken;
                //var JWToken = HttpContext.Current.Session["JWToken"].ToString();
                if (!string.IsNullOrEmpty(JWToken))
                {
                    context.Request.Headers.Add("Bearer ", JWToken);
                }
                else
                {
